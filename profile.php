<!DOCTYPE html>
<?php
$curr_us = isset($_GET['curr_us']) ? $_GET['curr_us'] : '';
$us = isset($_GET['profile_for']) ? $_GET['profile_for'] : '';
?>
...
if ($row = mysqli_fetch_array($result)) {
    ...
    $profile_picture = $row['profile_picture'] ?? ''; // Default value if null
    $bio             = isset($row['bio']) ? $row['bio'] : ''; // Default value if null
    $posts           = $row['posts']; // Default value if null
    $followers       = isset($row['followers']) ? $row['followers'] : ''; // Default value if null
    $followings      = $row['followings'] ?? ''; // Default value if null

    // Your existing HTML code that uses these variables
} else {
    // Handle case where the query result is empty (no matching user)
    echo "User not found.";
}
?>
...
<section class="profile">
    <header class="profile__header">
        <div class="profile__avatar-container">
            <img src="<?php echo $profile_picture == null ? 'images/avatar.svg' : $profile_picture; ?>" class="profile__avatar" />
        </div>
        <div class="profile__info">
            ...
            <ul class="profile__numbers">
                <li class="profile__posts">
                    <span class="profile__number u-fat-text"><?php echo $posts ?></span>
                </li>
